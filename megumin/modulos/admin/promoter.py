...
async def _promote_user(_, message: Message):
    ...
    if not await check_bot_rights(chat_id, "can_promote_members"):
        ...
        await sed_sticker(message)
        return
    sent = await message.reply("`Alterando título de administrador...`")
    try:
        if not args:
            await sent.edit("`Quer que eu adivinhe? Por favor forneça um título para que eu possa alterar`")
            return
        else:
            await asyncio.sleep(2)
        await megux.set_administrator_title(chat_id, user_id, args)
        await sent.edit("<b>Novo título de administrador definido com sucesso!!</b>\n<b>Titulo:</b> <code>{args}</code>")
    except Exception as e_f:
        await sent.edit(f"`Algo deu errado! 🤔`\n\n**ERROR:** `{e_f}`")
