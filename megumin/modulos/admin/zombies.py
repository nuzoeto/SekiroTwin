...
async def cleanup(c: megux, m: Message):
    if m.chat.type == "private":
        return await m.reply_text("Admin.err_private")
    else:
        ...
        if member.status in ["administrator", "creator"]:
            if bot.status in ["administrator"]:
                ...
                return await m.reply_text("Admin.botnotadmin")
        else:
            return await m.reply_text("Admin.noadmin")
    deleted_users = []
    sent = await m.reply_text("Admin.cleanup_start")
    async for a in c.iter_chat_members(chat_id=m.chat.id, filter="all"):
        if a.user.is_deleted:
            try:
                ...
                await sent.edit_text("Admin.cleanup_start_d {}").format(
                    {len(deleted_users)}
                )
            except BadRequest:
                pass
            ...
            await sent.edit_text("Admin.cleanup_no_deleted")
