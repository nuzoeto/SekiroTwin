...
from megumin.utils import EMOJI_PATTERN
...
async def kang_sticker(c: megux, m: Message):
  prog_msg = await m.reply_text("Aguarde..."))
    ...
    if reply and reply.media:
        if reply.photo:
            ...
            if not reply.sticker.file_name:
                return await prog_msg.edit_text("err_sticker_no_file_name")
    ...
    try:
        ...
        if packname_found:
            await prog_msg.edit_text("use_existing_pack")
            await c.send(
                AddStickerToSet(
                    stickerset=InputStickerSetShortName(short_name=packname),
            ...
            await prog_msg.edit_text("create_new_pack_string")
            ...
            try:
                ...
                return await prog_msg.edit_text("cant_create_sticker_pack_string"),
        ...
        markup = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("view_sticker_pack_btn"), url=f"t.me/addstickers/{packname}",
                    )
                ]
            ]
        )
        kanged_success_msg = 
        ("sticker_kanged_string")
